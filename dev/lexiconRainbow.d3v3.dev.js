(function (root,factory) {
	...
	function LexiconRainbow () {
		...
		function renderLeftPanel (containerOrdinal,containerLinear,containerImage) {
			...
			var obj = {subject:undefined,
						...
						changeImage:function(){
							...
							.each("end",function(){selection.attr("xlink:href",_input_.linear[linearID].glyph || "")})
							.style("opacity",0).delay(0).duration(250)
							.transition().style("opacity",0.7).duration(250);
						},
						changeText:function(nText){if(!this.hasText()){return;} this.subject.select("text").transition("changeText").each("end",function(){d3.select(this).text(nText)}).attr("fill-opacity",0).delay(0).duration(250)
			...
			slider.node().render = function(type,count,offset){
				...
				window.requestAnimationFrame(function(){
					...
					if(!that.__on__){
						...
						var drag = d3.behavior.drag().origin(function(){var x = +d3.select(this).attr("x"); var y = +d3.select(this).attr("y"); return {"x":x,"y":y};}).on("drag", dragFunc).on("dragstart",function (){this.__count__ = this.__count__ || count;this.__dCount__ = 0;this.__timer__.dragStarted();d3.event.sourceEvent.stopPropagation();}).on("dragend",dragEnd);
						function dragFunc() {
			...
			slider.node().remove = function() {
				...
				.each("end",function(){/*console.log(this);*/d3.select(this).attr("filter","none")}).delay(0).duration(1500);
		...
		this.unwarp = function (f) {
			...
			d3.select("#"+ID).transition("shrink").each("end",function(){d3.select(this).remove();if(typeof f === "function"){return f.bind(_this_)();}}).tween("unwarp",function(){var interpolator = d3.interpolate(viewBox,viewBoxFinal);return function(t){this.setAttribute("viewBox",interpolator(t))}}).delay(0).duration(3000);
	...
	prt.onpickStart = function(enableOnPick,e,oNode,attrW,attrH,width,scaleInTransition,request,handle,input,linearID,d,i) {
		...
		thisG.transition("draw").ease("linear").attr("transform",function(){return "translate("+(0.25*attrW+i*width)+",0)"}).delay(0).duration(250);
