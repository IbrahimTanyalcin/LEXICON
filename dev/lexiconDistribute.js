	...
	function lexiconDistributeF () {
		...
		function addLocal() {
			...
			this.updateIndicator = function(foreignCaller) {
				...
				this.highlight();
				//2 bars and a span region and 2 text showing intervals
				d3.select("#"+ID+"_indicatorLeftBar").attr("transform","translate("+(offset/length*attrW*0.9)+",0) scale("+attrW/30+","+attrH/30+")");
				d3.select("#"+ID+"_indicatorRightBar").attr("transform","translate("+(offset/length*attrW*0.9+spanWidth)+",0) scale("+attrW/30+","+attrH/30+")");
				d3.select("#"+ID+"_indicatorSpanRegion").attr("x",offset/length*attrW*0.9).attr("width",spanWidth);
				d3.select("#"+ID+"_indicatorLeftText").text(function(){return Math.floor(offset)+1;}).attr("transform","translate("+(offset/length*attrW*0.9)+",0)");
				d3.select("#"+ID+"_indicatorRightText").text(function(){return Math.min(Math.ceil(offset+N)+1,length);}).attr("transform","translate("+(offset/length*attrW*0.9+spanWidth)+",0)");
			}
